...
def parse_arguments():
    parser = argparse.ArgumentParser(description="使用 Attention U-Net 模型进行预测与测试。")
    parser.add_argument("--weight_path", required=True, help="要加载的 Attention U-Net 权重路径。")
...
def main():
    ...
    metrics_path = os.path.join(output_dir, "test_metrics_attention_bce.txt")
    with open(metrics_path, "w", encoding="utf-8") as file:
        file.write(result_text)

    prediction_dir = os.path.join(output_dir, "prediction_samples_attention_bce")
    os.makedirs(prediction_dir, exist_ok=True)
    sample_path = os.path.join(prediction_dir, "attention_prediction_samples_bce.png")
