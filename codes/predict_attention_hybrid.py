...
def parse_arguments():
    parser = argparse.ArgumentParser(description="使用 Attention U-Net + BCE + Dice 模型进行预测与测试。")
...
def main():
    ...
    result_text = (
        "========== D 组模型（Attention U-Net + BCE + Dice）测试集指标 ==========\n"
    ...
    metrics_path = os.path.join(output_dir, "test_metrics_attention_hybrid.txt")
    with open(metrics_path, "w", encoding="utf-8") as file:
        file.write(result_text)

    prediction_dir = os.path.join(output_dir, "prediction_samples_attention_hybrid")
    os.makedirs(prediction_dir, exist_ok=True)
    sample_path = os.path.join(prediction_dir, "attention_hybrid_prediction_samples.png")
