...
def parse_arguments():
    parser = argparse.ArgumentParser(description="使用 U-Net 基线模型进行预测与测试。")
    parser.add_argument("--weight_path", required=True, help="要加载的基线模型权重路径。")
...
def main():
    ...
    metrics_path = os.path.join(output_dir, "test_metrics_baseline.txt")
    with open(metrics_path, "w", encoding="utf-8") as file:
        file.write(result_text)

    prediction_dir = os.path.join(output_dir, "prediction_samples")
    os.makedirs(prediction_dir, exist_ok=True)
    sample_path = os.path.join(prediction_dir, "baseline_prediction_samples.png")
