...
MODEL_NAME = "unet_hybrid"


def parse_arguments():
    parser = argparse.ArgumentParser(description="训练 U-Net + BCE + Dice 混合损失模型（B 组）。")
    ...
    parser.add_argument(
        ...
        default=EPOCHS,
        help=f"最大训练轮数，默认 {EPOCHS}。",
    )
    parser.add_argument(
        "--patience",
        type=positive_integer,
        default=PATIENCE,
        help=f"早停耐心值，默认 {PATIENCE}。",
    )
    parser.add_argument(
        "--quick",
        action="store_true",
        help="快速模式：等价于 --epochs 25 --patience 5，适合 CPU 环境。",
    )
    arguments = parser.parse_args()
    if arguments.quick:
        arguments.epochs = 25
        arguments.patience = 5
    return arguments
...
def main():
    ...
    log_path = os.path.join(experiment_dir, "train_log_hybrid.csv")
    curve_path = os.path.join(experiment_dir, "train_curve_hybrid.png")
    ...
    print(f"使用设备: {device}")
    print(
        f"开始 B 组实验：U-Net + BCE + Dice | batch_size={arguments.batch_size}, "
    ...
    for epoch in range(1, arguments.epochs + 1):
        ...
        print(
            f"轮次 {epoch:03d}/{arguments.epochs} | 训练损失: {train_loss:.4f} | "
        ...
        if validation_dice > best_dice:
            ...
            if early_stop_counter >= arguments.patience:
                print(f"早停触发，连续 {arguments.patience} 轮未提升")
                break
    ...
    with open(config_path, "w", encoding="utf-8") as file:
        file.write(f"experiment_group: B\n")
        file.write(f"model: U-Net + BCE + Dice\n")
        ...
        file.write(f"epochs: {arguments.epochs}\n")
        file.write(f"patience: {arguments.patience}\n")
