...
def parse_arguments():
    parser = argparse.ArgumentParser(description="训练 U-Net 基线模型。")
...
def main():
    ...
    log_path = os.path.join(experiment_dir, "train_log_baseline.csv")
    curve_path = os.path.join(experiment_dir, "train_curve_baseline.png")
    ...
    print(f"使用设备: {device}")
    print(f"开始 A 组实验：U-Net + BCE 损失 | batch_size={arguments.batch_size}, lr={format_learning_rate(arguments.lr)}")
